...
import { Checkbox, IconButton, InputBase, Paper, Stack, TableCell, TableHead, TableSortLabel, Toolbar, Typography } from '@mui/material';
        ...
        fetch('/authors.json')
            ...
            .then(authorsData => {
                const authors = authorsData.map(author => ({
                    ...
                    books: [] // Initialize books array for each author
                }));
    ...
    const handleDelete = (bookId) => {
        setData(prevAuthors =>
            prevAuthors.map(author => ({
                ...author,
                books: author.books.filter(book => book.id !== bookId) // Remove book with matching id
            }))
        );
    };
                ...
                {filteredData.map((author) => {
                    return (
                        <ListPaper loading={loading} data={!!(filteredData && filteredData.length > 0)}>
                            <TableData
                                toolBar={<EnhancedTableToolbar name={author.name} />}
